...
def get_vehicle_pose_from_file(fname, uav_id):
    ...
    for row in array_string:
        if len(row) != 5:
            print(
                "Incorrect data in file '%s'! Data should be in format [id, x, y, z, heading] (example: int, float, float, float, float)"
                % fname
            )
            sys.exit(1)
        print(row)
...
def get_model_xacro_file(vehicle_type):
    ...
    try:
        ...
        print_error(
            "   Note: The selected model doesn't have to be publicly available."
        )
        print_error(
            "==================================================================================="
        )
    ...
    model_filename = os.path.join(model_pkg_path, "%s" % model_file)
...
def spawn_model(
    ...
    kwargs["mappings"]["enable_ground_truth"] = (
        "true" if enable_ground_truth else "false"
    )
    kwargs["mappings"]["enable_bluefox_camera"] = (
        "true" if enable_bluefox_camera else "false"
    )
    kwargs["mappings"]["enable_bluefox_camera_reverse"] = (
        "true" if enable_bluefox_camera_reverse else "false"
    )
    kwargs["mappings"]["enable_mobius_camera_down"] = (
        "true" if enable_mobius_camera_down else "false"
    )
    kwargs["mappings"]["enable_mobius_camera_front"] = (
        "true" if enable_mobius_camera_front else "false"
    )
    kwargs["mappings"]["enable_mobius_camera_back_left"] = (
        "true" if enable_mobius_camera_back_left else "false"
    )
    kwargs["mappings"]["enable_mobius_camera_back_right"] = (
        "true" if enable_mobius_camera_back_right else "false"
    )
    kwargs["mappings"]["enable_realsense_front_pitched"] = (
        "true" if enable_realsense_front_pitched else "false"
    )
    kwargs["mappings"]["enable_realsense_down"] = (
        "true" if enable_realsense_down else "false"
    )
    kwargs["mappings"]["enable_realsense_top"] = (
        "true" if enable_realsense_top else "false"
    )
    kwargs["mappings"]["enable_realsense_front"] = (
        "true" if enable_realsense_front else "false"
    )
    kwargs["mappings"]["use_realistic_realsense"] = (
        "true" if use_realistic_realsense else "false"
    )
    ...
    kwargs["mappings"]["enable_rangefinder_up"] = (
        "true" if enable_rangefinder_up else "false"
    )
    kwargs["mappings"]["enable_whycon_box"] = "true" if enable_whycon_box else "false"
    kwargs["mappings"]["enable_magnetic_gripper"] = (
        "true" if enable_magnetic_gripper else "false"
    )
    kwargs["mappings"]["enable_scanse_sweep"] = (
        "true" if enable_scanse_sweep else "false"
    )
    kwargs["mappings"]["enable_rplidar"] = "true" if enable_rplidar else "false"
    kwargs["mappings"]["enable_teraranger_tower_evo"] = (
        "true" if enable_teraranger_tower_evo else "false"
    )
    ...
    kwargs["mappings"]["enable_water_gun"] = (
        "true" if enable_water_gun and not fire_challenge else "false"
    )
    ...
    kwargs["mappings"]["enable_servo_camera"] = (
        "true" if enable_servo_camera else "false"
    )
    kwargs["mappings"]["enable_thermal_camera"] = (
        "true" if enable_thermal_camera else "false"
    )
    ...
    kwargs["mappings"]["fire_challenge_blanket"] = (
        "true" if fire_challenge_blanket else "false"
    )
    ...
    kwargs["mappings"]["enable_uv_leds_beacon"] = (
        "true" if enable_uv_leds_beacon else "false"
    )
    ...
    if not detect_unused_arguments_in_xacro(model_xml, vehicle_type, **kwargs):
        ...
        print_error(
            "   Please check possible settings for this type of UAV by calling command:"
        )
    ...
    try:
        ...
        print_error(
            '   Please run gazebo_ros (for example: "roslaunch mrs_simulation simulation.launch")'
        )
        print_error(
            "==================================================================================="
        )
...
def detect_unused_arguments_in_xacro(
    template_xml, vehicle_type, mappings=None, **kwargs
):
        ...
        if "enable" in key:
            ...
            if eval_value:
                ...
                if searchtext not in template_xml:
                    print_error(
                        "Setting --{} is not specified for {}".format(key, vehicle_type)
                    )
...
def generate_launch_file(mav_sys_id, vehicle_type, baseport, config_path):
    launch_snippet = get_launch_snippet(mav_sys_id, vehicle_type, baseport, config_path)
    return write_launch_file(launch_snippet)


def get_launch_snippet(mav_sys_id, vehicle_type, vehicle_base_port):
    data = {
...
def empy(template_name, data, options=None):
    ...
    try:
        ...
        print(
            "%s processing template '%s'" % (e.__class__.__name__, template_name),
            file=sys.stderr,
        )
        raise
    finally:
