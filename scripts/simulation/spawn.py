...
def spawn():
    parser = argparse.ArgumentParser(
        description="Spawn vehicles into gazebo simulation.",
    ...
    type_group.add_argument(
        "--f550", action="store_true", help="Force the spawned vehicle to DJI F550."
    )
    type_group.add_argument(
        "--t650",
        action="store_true",
    ...
    parser.add_argument(
        "--run", action="store_true", help="run generated launch file (default: false)"
    )
    parser.add_argument(
        "--delete", action="store_true", help="Despawn when killed (default: false)"
    )
    parser.add_argument(
        "--file",
    ...
    parser.add_argument(
        "--uv-camera-calibration-file",
        nargs=1,
        type=str,
        default=("~/calib_results.txt"),
    ...
    parser.add_argument(
        "--gazebo-ros-master-uri", help="The uri used to spawn the models"
    )
    parser.add_argument(
        "--mavlink-address", help="The IP address for mavlink (default: INADDR_ANY)"
    )
    parser.add_argument(
    ...
    if n_vehicles > 1:
        print(
            "\033[31;1m"
            + "[WARN]: Spawning multiple vehicles in one command is currently not supported.\n[WARN]: Only the first vehicle will spawn.\n"
            + "\033[32;1m"
            + "[HINT]: Create a separate tmux pane (ctrl+s) and call the script again to spawn more vehicles"
            + "\x1b[0m"
        )
        n_vehicles = 1
